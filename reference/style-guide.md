...
一句话：**黑底幕底（星点 + 底部雾底渐变，或点阵波）**上的白线条 MG。所有图形是「黑填充 + 白描边 2–3px」或「青绿/橙纯色块 + 白描边」；中文超粗黑体（Noto Sans SC 900，常 scaleX .8–.85 压窄）；英文技术词青绿色粗斜体 Exo 2；常驻顶部青绿胶囊 HUD、底部 44px 白字黑边字幕、半透明章节进度条。历史样片为紫色，只参考构图与节奏；新片颜色以当前模板令牌为准。
...
| 顶部 HUD 胶囊 | (533,28,216,51) r25.5，青绿 #14B8A6 填充 + 白 2px 边，33px 700 白字；英文副标 Exo 2 30px 青绿在 y≈94 | 覆盖层绘制；换词 8 帧淡入 |
...
| 进度条 | y687–720 半透明：已播 rgba(20,184,166,.52)、未播 rgba(243,243,243,.32)；n−1 根白分隔线；章名 Noto 900 24px skewX(−10°) scaleY(.9)，当前章 α1 其余 .55 | 内容穿过会被提亮，只允许全幅背景/大图形穿过 |
| z 序 | 黑底 < 幕底（Fog + 星点，或 DotFieldBg）< 实拍 < 覆盖层 < 镜头 < 片尾压黑 < 进度条 < aboveBar 镜头 < 字幕 | |

## 2. 调色板（`src/ui.tsx` 常量）
| 名 | 值 | 语义 |
|---|---|---|
| PURPLE | #14B8A6 | 当前重点 / 激活 / 品牌；兼容旧代码的变量名 |
| PURPLE_LIGHT | #5EEAD4 | 高光端、active 卡边 |
| PURPLE_TECH | #2DD4BF | Exo 2 英文技术词 |
| PURPLE_DEEP | #0F766E | 曲线、标题硬投影 |
...
**颜色即语义**：青绿=重点/我们的、灰=未激活/背景、橙红=指标/警示/对方、白=结构文字、绿=正确。一帧里青绿色重点不超过一处大面积。
