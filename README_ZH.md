...
**科卡二创实战样片**——《AI 风险与文明选择》，基于公开公众号文章制作。成片 5′00″、48 句 / 1279 字，黑底 MG 动画，使用火山引擎 TTS 2.0 按自然段连贯合成、本地 Whisper 字幕对齐和跨句连续画面。它展示了这套 Skill 从文章归档、事实核查、解说改写到配音、字幕、动画与 QC 的完整中文工作流。

[▶ 播放 / 下载 300 秒完整成片](media/keka-ai-risk-volcengine-tts2.mp4)

仓库保留的 [`examples/rag/`](examples/rag/) 是上游模板过程资料，用于展示调研、解说词、分镜、镜头源码、QC 与交付文件的组织方式；当前首页质量样片以科卡的《AI 风险与文明选择》为准。
...
**支持哪些语言？**
中文和英文。各有自己的语速模型、字幕预算与默认音色。页面顶部链接的是科卡中文二创样片；`examples/rag/` 保留上游模板的过程资料。
...
- 中英文都支持（`lang: 'zh' | 'en'`），各有自己的语速、字幕块预算（每块 16 字 / 48 字符）与默认音色。上方链接科卡中文二创样片；`examples/rag/` 保留上游模板过程资料。只做这一种视觉风格，幕底二选一（`bg: 'stars' | 'dots'`），其他要换就改 `reference/style-guide.md` + `src/ui.tsx`。
